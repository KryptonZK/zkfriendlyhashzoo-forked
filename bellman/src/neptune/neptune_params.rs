...
impl<S: PrimeField> NeptuneParams<S> {
    ...
    fn instantiate_rc(t: usize, rounds: usize, shake: &mut Sha3XofReader) -> Vec<Vec<S>> {
        (0..rounds)
            .map(|_| {
                (0..t)
    ...
    fn instantiate_abc(shake: &mut Sha3XofReader) -> [S; 3] {
        let mut abc = [S::one(); 3];
        abc[2] = utils::field_element_from_shake_without_0(shake);
        abc
    }

    fn instantiate_mu(t: usize, shake: &mut Sha3XofReader) -> Vec<S> {
    ...
    fn instantiate_external_matrix(t: usize, shake: &mut Sha3XofReader) -> Vec<Vec<S>> {
        let t_ = t >> 1;
