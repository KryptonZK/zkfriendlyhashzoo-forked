...
impl<S: PrimeField> Permutation<S> for Neptune<S> {
...
mod neptune_tests_bls12 {
    ...
    fn build_mi(neptune_params: &Arc<NeptuneParams<Scalar>>) -> Vec<Vec<Scalar>> {
        ...
        let mut mi = vec![vec![Scalar::one(); t]; t];
        for (i, mii) in mi.iter_mut().enumerate() {
            mii[i] = neptune_params.mu[i];
            mii[i].add_assign(&Scalar::one()); // Compensate for subtraction in parameter creation
        }
        mi
    }
    ...
    fn matmul_equalities(t: usize) {
        ...
        for (row, merow) in me.iter().enumerate() {
            for (col, v) in merow.iter().enumerate() {
                if (row + col) % 2 == 0 {
    ...
    fn consistent_perm() {
        // TODO use existing
        let neptune_params = Arc::new(NeptuneParams::<Scalar>::new(14, 3, 2, 1));
        let neptune = Neptune::new(&neptune_params);
        let t = neptune.params.t;
        for _ in 0..TESTRUNS {
