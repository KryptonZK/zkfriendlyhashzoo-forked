...
use hash_r1cs::{
    ...
    grendel::{
        grendel::Grendel, grendel_circuit::GrendelCircuit,
        grendel_instance_bls12::GRENDEL_BLS_3_PARAMS,
    },
...
fn main() {
    println!("Grendel proof (t = 3)");
    let grendel = Grendel::new(&GRENDEL_BLS_3_PARAMS);
    let grendel_circuit = GrendelCircuit::new(&GRENDEL_BLS_3_PARAMS);
    let mut rng = thread_rng();
    let mut groth = PermGroth::new(grendel_circuit);
    println!("Create CRS");
    ...
    let result = PermGroth::<Bls12, GrendelCircuit<Bls12>>::verify_proof(&pvk, &proof, &perm);
