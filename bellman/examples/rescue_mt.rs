...
fn main() {
    println!("Rescue MT proof (2:1, 2^10 elements)");
    let perm = Rescue::new(&RESCUE_BLS_3_PARAMS);
    ...
    let perm_circ = RescueCircuit::new(&RESCUE_BLS_3_PARAMS);
    ...
    let result = MerkleGroth::<Bls12, RescueCircuit<Bls12>>::verify_proof(
        &pvk,
        &proof,
        &mt.get_root().unwrap(),
    );
