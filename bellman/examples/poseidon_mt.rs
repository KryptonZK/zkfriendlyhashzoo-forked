...
fn main() {
    println!("Poseidon MT proof (2:1, 2^10 elements)");
    ...
    let perm_circ = PoseidonCircuit::new(&POSEIDON_BLS_3_PARAMS);
    ...
    let result = MerkleGroth::<Bls12, PoseidonCircuit<Bls12>>::verify_proof(
        &pvk,
        &proof,
        &mt.get_root().unwrap(),
    );
