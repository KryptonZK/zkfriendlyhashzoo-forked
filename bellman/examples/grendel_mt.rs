...
fn main() {
    println!("Grendel MT proof (2:1, 2^10 elements)");
    let perm = Grendel::new(&GRENDEL_BLS_3_PARAMS);
    ...
    let perm_circ = GrendelCircuit::new(&GRENDEL_BLS_3_PARAMS);
    ...
    let result = MerkleGroth::<Bls12, GrendelCircuit<Bls12>>::verify_proof(
        &pvk,
        &proof,
        &mt.get_root().unwrap(),
    );
