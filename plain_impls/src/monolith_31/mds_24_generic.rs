...
fn mds_multiply_generic<F: Field32 + PrimeField>(state: &mut [F; 24]) {
    ...
    lo = fast_cyclomul32(&lo, MDS_MATRIX_FIRST_COLUMN);
    hi = fast_cyclomul32(&hi, MDS_MATRIX_FIRST_COLUMN);
...
#[inline(always)]
fn fast_cyclomul32(f: &[u64; 32], g: [i64; 32]) -> [u64; 32] {
    const N: usize = 16;
    ...
    let hh_lo = fast_cyclomul16(ff_lo, gg_lo);
    let hh_hi = complex_negacyclomul16(ff_hi, gg_hi);
...
#[inline(always)]
fn complex_negacyclomul16(f: [i64; 16], g: [i64; 16]) -> [i64; 16] {
    const N: usize = 8;
    ...
    let h0 = complex_karatsuba8(f0, g0);
...
#[inline(always)]
fn complex_karatsuba8(f: [(i64, i64); 8], g: [(i64, i64); 8]) -> [(i64, i64); 15] {
    const N: usize = 4;

    let ff = complex_sum::<4>(f[..N].try_into().unwrap(), f[N..].try_into().unwrap());
    let gg = complex_sum::<4>(g[..N].try_into().unwrap(), g[N..].try_into().unwrap());
