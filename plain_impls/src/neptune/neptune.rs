...
mod neptune_tests_bls12 {
    ...
    fn build_mi(neptune_params: &Arc<NeptuneParams<Scalar>>) -> Vec<Vec<Scalar>> {
        ...
        for (i, matrow) in mi.iter_mut().enumerate().take(t) {
            ...
            matrow[i].add_assign(&Scalar::one()); // Compensate for subtraction in parameter creation
        }
        mi
    }
    ...
    fn matmul_equalities(t: usize) {
        ...
        for (row, matrow) in me.iter().enumerate().take(t) {
            for (col, matrowcol) in matrow.iter().enumerate().take(t) {
                if (row + col) % 2 == 0 {
                    assert!(*matrowcol != Scalar::zero());
                } else {
                    assert_eq!(*matrowcol, Scalar::zero());
                }
            }
        }
...
#[cfg(test)]
mod neptune_tests_st {
    use super::*;
    use crate::{
        fields::{st::FpST, utils},
        neptune::neptune_instances::NEPTUNE_ST_PARAMS,
    };
    use ff::Field;
    type Scalar = FpST;
    ...
    #[test]
    fn consistent_perm() {
        let neptune = Neptune::new(&NEPTUNE_ST_PARAMS);
        let t = neptune.params.t;
        for _ in 0..TESTRUNS {
            let input1: Vec<Scalar> = (0..t).map(|_| utils::random_scalar(true)).collect();
