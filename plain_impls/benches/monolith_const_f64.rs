...
use zkhash::{
    fields::{const_f64::ConstF64, f64::Field64, utils},
    monolith_64::{
        monolith_64::Monolith64,
        monolith_64_instances::{MONOLITH_CONST64_12_PARAMS, MONOLITH_CONST64_8_PARAMS},
    },
};
...
fn permutation(c: &mut Criterion) {
    ...
    c.bench_function("Monolith ConstF64 Permutation t=8", move |bench| {
        bench.iter(|| {
...
fn permutation_u128(c: &mut Criterion) {
    ...
    c.bench_function("Monolith ConstF64 Permutation t=8 (u128)", move |bench| {
        bench.iter(|| {
...
fn permutation_12(c: &mut Criterion) {
    ...
    c.bench_function("Monolith ConstF64 Permutation t=12", move |bench| {
        bench.iter(|| {
            let perm = monolith.permutation_(black_box(&input));
            black_box(perm)
        });
...
fn permutation_u128_12(c: &mut Criterion) {
    ...
    c.bench_function("Monolith ConstF64 Permutation t=12 (u128)", move |bench| {
        bench.iter(|| {
            let perm = monolith.permutation_u128(black_box(&input));
            black_box(perm)
        });
...
fn hash(c: &mut Criterion) {
    ...
    c.bench_function("Monolith ConstF64 Hash", move |bench| {
        bench.iter(|| {
...
fn concrete(c: &mut Criterion) {
    ...
    c.bench_function("Monolith ConstF64 Concrete t=8", move |bench| {
        bench.iter(|| {
...
fn concrete_12(c: &mut Criterion) {
    ...
    c.bench_function("Monolith ConstF64 Concrete t=12", move |bench| {
        bench.iter(|| {
            let mut perm = input.to_owned();
            monolith.concrete_u128(black_box(&mut perm), black_box(&round_const));
            black_box(perm)
...
fn bricks(c: &mut Criterion) {
    ...
    c.bench_function("Monolith ConstF64 Bricks t=8", move |bench| {
        bench.iter(|| {
            let mut perm = input.to_owned();
...
fn bricks_12(c: &mut Criterion) {
    ...
    c.bench_function("Monolith ConstF64 Bricks t=12", move |bench| {
        bench.iter(|| {
            let mut perm = input.to_owned();
...
fn bars(c: &mut Criterion) {
    ...
    c.bench_function("Monolith ConstF64 Bars t=8", move |bench| {
        bench.iter(|| {
            let mut perm = input.to_owned();
...
fn bars_12(c: &mut Criterion) {
    ...
    c.bench_function("Monolith ConstF64 Bars t=12", move |bench| {
        bench.iter(|| {
            let mut perm = input.to_owned();
...
fn bar(c: &mut Criterion) {
    ...
    c.bench_function("Monolith ConstF64 Bar", move |bench| {
        bench.iter(|| {
