...
fn griffin_permutation(c: &mut Criterion) {
    let rc = Griffin::new(&GRIFFIN_BLS_PARAMS);
    let input: [Scalar; 3] = [
        utils::random_scalar(true),
        utils::random_scalar(true),
        utils::random_scalar(true),
    ...
    c.bench_function("Griffin BLS12 Permutation", move |bench| {
        bench.iter(|| {
            let perm = rc.permutation(black_box(&input));
            black_box(perm)
        });
...
fn neptune_permutation(c: &mut Criterion) {
    let rc = Neptune::new(&NEPTUNE_BLS_PARAMS);
    let input: [Scalar; 4] = [
        utils::random_scalar(true),
        utils::random_scalar(true),
        utils::random_scalar(true),
    ...
    c.bench_function("Neptune BLS12 Permutation", move |bench| {
        bench.iter(|| {
            let perm = rc.permutation(black_box(&input));
            black_box(perm)
        });
